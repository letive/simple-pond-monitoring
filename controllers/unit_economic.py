...
from lib.unit_economic import cost_structure
...
def base_section():
    ...
    alpha = (
        st.sidebar.number_input(
            "alpha (shrimp growth rate)", value=1.0, step=1.0, format="%.2f"
        )
        / 100
    )
    ...
    e = st.sidebar.number_input("energy day cost", value=4.0, step=1.0, format="%.2f")
    ...
    if submit:
        ...
        if partial4 >= 0:
            ...
            with col1:
                option2 = Line(
                    "Biomassa", index, [data["revenue"]["biomassa"]], ["Biomassa (kg)"]
                ).plot()
            ...
            with col2:
                option3 = Line(
                    "Revenue",
                    index,
                    [data["revenue"]["revenue"], data["revenue"]["potential_revenue"]],
                    ["Realized Revenue", "Potential Revenue"],
                    True,
                ).plot()
            ...
            with col11:
                dataviz = [
                    {"value": i[1], "name": data["index"][i[0]]}
                    for i in enumerate(data["aggregate"])
                ]
            ...
            with col12:
                ...
                option5 = Line(
                    ...
                    index,
                    [
                        data["data_profit"]["revenue"],
                        data["data_profit"]["cost"],
                        data["data_profit"]["profit"],
                    ],
            ...
            st.warning(
                "Your partial harvest was wrong, the sum of partials is {} not matched".format(
                    round(partial1 * n0)
                    + round(partial2 * n0)
                    + round(partial3 * n0)
                    + round(partial4 * n0)
